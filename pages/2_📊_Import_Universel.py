...
st.caption(
    "Charge n'importe quel fichier Excel ou CSV standard pour l'analyser, créer"
    " des graphiques personnalisés et explorer les statistiques."
)

# Zone de dépôt de fichier
uploaded_file = st.file_uploader(
    "Dépose ton fichier (Excel .xlsx, .xls ou CSV)", type=["xlsx", "xls", "csv"]
)
...
if uploaded_file is not None:
  try:
    ...
    st.success(
        f"✅ Fichier **{uploaded_file.name}** chargé avec succès !"
    )
    ...
    # 3. ONGLETS D'ANALYSE
    tab_graph, tab_stats, tab_data = st.tabs([
    ...
    with tab_graph:
      ...
      with c3:
        ...
        col_y = st.selectbox(
            "Axe Y (Verticale) :",
            options=options_y,
            index=min(1, len(options_y) - 1),
        )

      # Option avancée : Couleur / Groupement
      col_color = st.selectbox(
          "Grouper/Colorer par (Optionnel) :", options=["Aucun"] + list(df.columns)
      )
      ...
      try:
        ...
        st.error(
            "Impossible de générer ce type de graphique avec les colonnes"
            f" sélectionnées : {chart_err}"
        )
    ...
    with tab_data:
      ...
      search = st.text_input(
          "🔍 Rechercher un mot-clé dans les données :", ""
      )
    ...
    # --- BLOC PERMANENT : TRAITEMENT & PRÉDICTION IA (HORS DES ONGLETS) ---
    st.markdown("---")
    st.subheader("⚡ Traitement & Prédiction IA")

    col_save, col_pred = st.columns(2)

    with col_save:
      # 1. ENREGISTREMENT DANS SUPABASE
      if st.button(
          "💾 1. Envoyer les données vers Supabase", use_container_width=True
      ):
        try:
          records = df.to_dict(orient="records")
          supabase.table("releves_meteo").insert(records).execute()
          st.success("✅ Données enregistrées dans Supabase avec succès !")
        except Exception as e:
          st.error(f"Erreur d'enregistrement Supabase : {e}")

    with col_pred:
      # 2. CALCUL DE LA PRÉDICTION IA
      if st.button(
          "🤖 2. Calculer le Risque d'Inondation", use_container_width=True
      ):
        col_rain = next(
            (
                c
                for c in df.columns
                if "precip" in c.lower() or "pluie" in c.lower()
            ),
            None,
        )

        if col_rain:
          max_pluie = df[col_rain].max()
          cumul_pluie = df[col_rain].sum()

          st.markdown("**Résultat de l'analyse IA :**")
          if max_pluie > 50 or cumul_pluie > 100:
            st.error(
                "🚨 **Alerte Rouge : Risque Fort d'Inondation** (Pic détecté :"
                f" {max_pluie:.1f} mm)"
            )
          elif max_pluie > 20:
            st.warning(
                "🟡 **Alerte Jaune : Vigilance Modérée** (Pic détecté :"
                f" {max_pluie:.1f} mm)"
            )
          else:
            st.success(
                "🟢 **Situation Normale : Risque Faible** (Cumul :"
                f" {cumul_pluie:.1f} mm)"
            )
        else:
          st.info(
              "ℹ️ Aucune colonne 'Pluie' ou 'Précipitation' identifiée pour"
              " évaluer le risque."
          )
